...
namespace Gamedonia.Backend
{
		...
		private static void _reauthorizeSessionWithPublishPermissions(string [] permissions, int size) {
			...
			private static void _publishInstall(string appId) {
				...
				public static IEnumerator _isFacebookTokenValid (string accessToken, Action<bool> callback = null)
				{
						

						if (accessToken != null && !accessToken.Equals ("")) {
							
								Request request = new Request ("GET", "https://graph.facebook.com/me?fields=id&access_token=" + accessToken);
								...
								if (GamedoniaBackend.INSTANCE.debug) Debug.Log("[Facebook Graph][validate AccessToken] - " + request.response.status + " " + request.response.Text);

								callback (request.response.status == 200);
